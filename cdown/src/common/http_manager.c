...
static int RetrieveOutputNameFromContentDisposition(RequestContext *request_context,
                                                    ...
                                                    size_t length) {
  /* Example Content-Disposition: attachment; filename=name1367; charset=funny; option=strange */
  /* If filename is present */
  content_disposition = strncasestr(content_disposition, FILENAME_KEY, FILENAME_KEY_LENGTH);
  ...
  int i = 0;
  while (i < length && content_disposition[i] != '\r' && content_disposition[i] != '\0' && content_disposition[i] != ';'
...
static void RetrieveOutputNameFromUrl(RequestContext *context) {
  char output_filename[MAX_FILENAME_LENGTH];
  const char *url = context->url;
  // http://.../三国演义.txt?param=1
  // http://.../三国演义.txt#chapter1
  // http://.../%E4%B8%89%E5%9B%BD%E6%BC%94%E4%B9%89.txt?param=1
  url = strrchr(url, '/') + 1;
  int i = 0;
  while (*url != '\0' && *url != '?' && *url != '#') {
