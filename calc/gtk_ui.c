...
static void DisplayTextWithGtkLabel(char *text) {
  gtk_label_set_text(ui_context->text_display_to, text);
}

static void OnInputButtonClicked(void *widget, void *data) {
  HandleInput(ui_context->calc_ctx, ((char *) data)[0]);
}
...
static void GtkUiMain(int argc, char **argv) {
  ...
  if (gtk_css_provider_load_from_path(css_provider, "gtk/calculator.css", &error) == 0) {
  ...
  if (gtk_builder_add_from_file(builder, "gtk/calculator.ui", &error) == 0) {
  ...
  ui_context->text_display_to = (GtkLabel *) gtk_builder_get_object(builder, "display");
  ...
  HandleInput(ui_context->calc_ctx, '0');
...
static void InitGtkUi() {
  ...
  ui_context->calc_ctx->display = DisplayTextWithGtkLabel;
}
