...
int CopyFile3(char const *src, char const *dest) {
  ...
  while (1) {
    size_t bytes_read = fread(buffer, sizeof(*buffer), BUFFER_SIZE, src_file);
    if (fwrite(buffer, sizeof(*buffer), bytes_read, dest_file) < bytes_read) {
      result = COPY_DEST_WRITE_ERROR;
      break;
    }
