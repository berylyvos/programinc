...
int CopyFileByChar(char const *src, char const *dest) {
  if (!src || !dest) {
...
int CopyFileByLine(char const *src, char const *dest) {
  if (!src || !dest) {
    return COPY_ILLEGAL_ARGUMENTS;
  }
...
int main() {
  ...
  // data/三国演义.txt costs: 61
  for (int i = 0; i < 2; ++i) {
    TimeCost(NULL);
    CopyFileByChar(srcs[i], dsts[i]);
    TimeCost(srcs[i]);
    PRINT_IF_ERROR("CopyFileByChar: %s", srcs[i]);
  }

  // data/io_utils.h costs: 0
  // data/三国演义.txt costs: 3
  for (int i = 0; i < 2; ++i) {
    TimeCost(NULL);
    CopyFileByLine(srcs[i], dsts[i]);
    TimeCost(srcs[i]);
    PRINT_IF_ERROR("CopyFileByLine: %s", srcs[i]);
  }
